/******w**************
    ...
    Name: Abubakar Abdulai
    Date: July 4. 2024
    Description: js for Assignment4
...
function load()
{
    ...
    // Attach event listeners to each 'query' class link
    var queryLinks = document.querySelectorAll('.query');
    for (var i = 0; i < queryLinks.length; i++) {
        queryLinks[i].addEventListener('click', function(event) {
            event.preventDefault();
            var substance = event.target.textContent;
            document.getElementById('searchInput').value = substance;
        });
    }
...
function searchDataSet(e)
{
    ...
    if(searchTerm != null && searchTerm != "")
    {
        ...
        // fetch the data
        // fetch("https://data.winnipeg.ca/resource/6x82-bz5y.json?$WHERE=substance)
        fetch(encodedURL)
            // .then(handleErrors)
...
function showExplanation(count, searchTerm)
{
    ...
    if (count > 0)
    {
        explanation.innerHTML = `The <strong>${count}</strong> most recent patient's description of events involving '' <strong>'${searchTerm}'</strong>.`;
    }
    else if(count == null)
    {
